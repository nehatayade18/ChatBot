...
starspace_embeddings, embeddings_dim = load_embeddings('week3/word_embeddings.tsv')
...
counts_by_tag = counts_by_tag = posts_df.groupby(by=['tag'])["tag"].count().reset_index(name = 'count').sort_values(['count'], ascending = False)######### YOUR CODE HERE #############
...
for tag, count in counts_by_tag.items():
    ...
    for i, title in enumerate(tag_posts['title']):
        tag_vectors[i, :] =tag_vectors[i, :] = question_to_vec(title, starspace_embeddings, embeddings_dim)
        ######### YOUR CODE HERE #############
